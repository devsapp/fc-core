...
export class HttpsCertConfig {
  ...
  static async getCertKeyContent(
    certKey: string = "",
    opt: Opt = {}
  ): Promise<string> {
  ...
  static async getUserCertificateDetail(certId: number, opt: Opt = {}) {
    ...
    if (core.lodash.isEmpty(credentials)) {
      throw new CatchableError(
        "You need to enter a key to get the information of the certificate"
      );
    }
  ...
  private static async getOSSContent(
    certKey: string,
    opt: Opt
  ): Promise<string> {
    ...
    if (core.lodash.isEmpty(credentials)) {
      throw new CatchableError(
        "You need to enter a key to get the content of OSS"
      );
    }
    const { AccessKeyID, AccessKeySecret, SecurityToken } = credentials;
